...
| BL-4 | Composed-view redesign — order: Trace (six pills) → Send/Step controls → Manual-mode overlay; fold out the Pin (removes the 3× active-step label). Absorbs the empty-header check (BL-9): no header renders over empty space. Panel-worthy — touches #3's GOLD role | Medium | Cycle 314.6 |
...
| BL-8 | Add a "data landed" moment after Stock the Pantry (persistence signal — "now in the database"). Deferred to Cycle 316, gated on the persistence layer (#8–#10) | Medium | Cycle 314.6 |
